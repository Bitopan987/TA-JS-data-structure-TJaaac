...
for (let movie of movies) {
  if (movie.genres.length > 1) {
    console.log(movie.title, "more than 1 genre");
  }
}
...
for (let movie of movies) {
  if (movie.actors.length === 3) {
    console.log(movie.title, "name equal to 3");
  }
}
// 30. Log the name of all the movies with year `2018`
for (let movie of movies) {
  if (movie.year === 2018) {
    console.log(movie.title, "name in 2018");
  }
}

// 31. Store the title of all the movies in a array named allTitle.
let allTitle = [];
for (let movie of movies) {
  allTitle.push(movie.title);
}
// 31. Store the genre of all the movies in a array named allGenre.
let allGenre = [];
for (let movie of movies) {
  for (let genre of movie.genres) {
    allGenre.push(genre);
...
// 31. Store the actor of all the movies in a array named allActor.
let allActor = [];
for (let movie of movies) {
  for (let actor of movie.actors) {
    allActor.push(actor);
...
// 33.Log the length of allActor,allGenre,allTitle
console.log(allActor.length, allGenre.length, allTitle.length);
